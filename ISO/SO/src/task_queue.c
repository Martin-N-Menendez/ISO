...
bool_t task_stack_pop 			(task_stack_t * task_stack,uint32_t * data ){
	...
	task_stack->size--;
	task_stack->read_pointer = ((task_stack->read_pointer+1)%TASK_STACK_SIZE);
	return TRUE;
}
bool_t task_stack_push			(task_stack_t * task_stack,uint32_t   data ){
	if(task_stack->size >= TASK_STACK_SIZE){
		return FALSE;
	}
	task_stack->data[task_stack->write_pointer] = data;
	task_stack->size++;
	task_stack->write_pointer = ((task_stack->write_pointer+1)%TASK_STACK_SIZE);
	return TRUE;
}
...
bool_t tack_stack_remove_item(task_stack_t * task_stack,uint32_t item){
	...
	if(task_stack_empty(task_stack) == FALSE){
